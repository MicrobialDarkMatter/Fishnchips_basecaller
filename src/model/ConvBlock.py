...
class ConvolutionBlock(tf.keras.layers.Layer):
    def __init__(self, cnn_dims, filters, idx, rate):
        ...
        self.dropout_layers = [tf.keras.layers.Dropout(rate) for _ in cnn_dims]
        self.activation_layer = tf.keras.layers.Activation('relu', name=f"res{idx}-relu")
        
    def call(self, x, training):
        res = x
        for cnn_layer, bn_layer, dropout_layer in zip(self.cnn_layers, self.bn_layers, self.dropout_layers):
            x = cnn_layer(x)
            x = bn_layer(x)
            x = dropout_layer(x, training)
